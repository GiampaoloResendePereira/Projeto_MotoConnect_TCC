...
function CalculoFrete() {
  ...
  const [telefoneOrigem, setTelefoneOrigem] = useState('');
  ...
  const validarCepJuizDeFora = async (cep) => {
    if (!validarCep(cep)) return false;
    try {
      const response = await axios.get(`https://viacep.com.br/ws/${cep}/json/`);
      return response.data.localidade === 'Juiz de Fora' && response.data.uf === 'MG';
    } catch {
      return false;
    }
  };
  ...
  const handleCepOrigemChange = async (e) => {
    ...
    if (await validarCepJuizDeFora(cep)) {
      try {
        const response = await axios.get(`https://viacep.com.br/ws/${cep}/json/`);
      ...
      setEnderecoOrigem('');
      setBairroOrigem('');
      setError('CEP de origem inválido. Apenas CEPs de Juiz de Fora, MG são permitidos.');
    }
  };
  ...
  const handleCepDestinoChange = async (e) => {
    ...
    if (await validarCepJuizDeFora(cep)) {
      ...
      setEnderecoDestino('');
        ...
        setTimeout(() => {
            setError('CEP de destino inválido. Apenas CEPs de Juiz de Fora, MG são permitidos.');
        }, 500); // 500 ms de delay
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Validações
    if (
      !validarCep(cepOrigem) || !validarCep(cepDestino) || !peso || !largura || !altura || !comprimento ||
